...
class Data_Producer_End_to_End(object):
      def conv_layer(self, input_data, filter_size, channels_in, channels_out, strides, conv_layer_dropout, name="Conv"):
            ...
            return tf.nn.elu(tf.nn.dropout(tf.nn.conv2d(input=input_data, filter=W, strides=strides, padding='SAME', use_cudnn_on_gpu=True), conv_layer_dropout))
      ...
      def _convolutional_feature_extractor(self, stft, conv_layer_dropout):
            ...
            with tf.variable_scope("Convbb", reuse=tf.AUTO_REUSE, initializer=self.init):
                  conv1 = self.conv_layer(input_data=stft, filter_size=8, channels_in=1, channels_out=32, strides=[1, 2, 2, 1], conv_layer_dropout=conv_layer_dropout, name="conv1")
