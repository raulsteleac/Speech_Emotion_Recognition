...
\begin{document}
	...
	\chapter{Analiza stadiului actual în domeniul problemei} \label{introdP}
		...
		\chapter{Descrierea implementării}
				...
				\section{Prezenatrea implementării sistemului de recunoaștere a emoțiilor}
...
class Data_Producer_End_to_End(object):
	...
	def _convolutional_feature_extractor(self, stft, conv_layer_dropout):
			...
			\subsection{Clasificatorul sistemului de recunoaștere a emoțiilor în vorbire} \label{clasif_prac}
				...
				\begin{lstlisting}[language=Python, caption={Implementarea celulelor LSTM bidirecționale pe doua nivele folosind procedurile specifice ale librariei Tensorflow.}, xleftmargin=0cm]
...
def create_LSTM_layer(self, inputs, hid_size, name=None):
	with tf.variable_scope(name):
		...
		return tf.concat(outputs, 2)[0]				\end{lstlisting} \par
...
%---------------------------------------------------------------------------------------------------------------------------------------------		
		\chapter{Rezultate si experimente} \label{rezultate}
		 ...
		 \begin{table}[t] \centering
		 	\caption{Compararea acurateții înregistrate antrenând modelul propus pe bazele de date: EMO-DB, RAVDESS, EMOVO, ENTERFACE'05 și pe întregul set de baze de date, alcătuit din cele patru enumerate anterior, JL, MAV si un set de înregistrări proprii. Antrenarea a fost realizata în două moduri, pe întregul set de emoții al fiecărei baze de date, cât și pe cele patru emoții principale: enervare, tristețe, fericire și neutru. Tabela este reprezentativă pentru primul experiment, în care se folosesc diferite baze de date pentru antrenare, dar arhitectura modelului rămâne cea propusă, "end-to-end".}
		 	\begin{tabular}{ccc}
		 		\hline
		 		Baza de date &  
		 		...
		 		&
		 		\bgroup	\def\arraystretch{1}
		 		\begin{tabular}{@{}c@{}} Acuratețea pe  \\  cele patru emoții principale \end{tabular} \egroup
		 		\\ \hline
		 		EMO-DB       & 77\%                   & 91.04\%      \\
		 		RAVDESS      & 71.08\%                & 76.11\%      \\
		 		EMOVO        & 70\%                   & 80.59\%      \\
		 ...
		 \par		 
		 Cel de al doilea experiment a fost realizat prin modificare modului de extragere a caracteristicilor de intrare. Metoda "end-to-end", folosirea rețelelor neuronale profunde pentru extragerea caracteristicilor de intrare, este cea propusă pentru arhitectura finală a sistemului SER implementat în această lucrare. Totuși pentru a evidenția importanța acestei tehnici, în continuare vor fi prezentate rezultatele înregistrate în urma antrenării modelului Machine Learning pe caracteristici de intrare extrase prin metoda tradițională, "hand-crafted". Metoda "hand-crafted" implica folosirea unor funcții matematice, precum coeficienții Mel sau delta și delta-deltas, pentru extragerea informației emoționale. Totuși încă nu s-a descoperit un set de caracteristici care să surprindă perfect informația emoțională dintr-o înregistrare audio. Acest lucru face că metoda "end-to-end" să fie mai atrăgătoare, deoarece rețelele neuronale profunde au potențialul să extragă un set de caracteristici mult mai subiectiv problemei de recunoaștere de emoții. Acuratețea maximă înregistrată în cadrul acestui experiment, folosirea caracteristicilor "hand-crafted" la antrenare, a fost de 68.56\%.
